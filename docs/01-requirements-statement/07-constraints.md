...
| DB | PostgreSQL — **시스템마다 인스턴스 1개씩**. 정산 서버는 정산 DB에만 접속한다 (🚧 제품은 AWS Aurora / Supabase 중 미정) |
| 배치 | Spring Batch (정산 서버 내부), 트리거는 Spring `@Scheduled` |
| 패키징 | Jar |
| 배포 | AWS EC2 1대의 Docker 컨테이너 3개 중 하나 (`ARCH §5`) |
...
| 날짜 | 내용 |
|---|---|
| 08.11(화) | ✅ 중간 회의 — DB 분리·이슈 관리·Projects 보드 결정 |
| 08.15(토) | 2차 회의 — 남은 🚧 항목 결정 |
