...
정산 서버가 **소유하는** 테이블이며, **정산 DB 인스턴스에만 존재한다** (`ERD §2`, `ARCH §1`).

DB가 분리돼 정산의 `ALTER TABLE`이 다른 두 사람의 로컬을 깨뜨리지는 않는다. 다만 스키마 변경이 **조회 API 응답 필드로 새어나가면** 소비자가 영향을 받으므로, 응답에 노출되는 컬럼을 바꿀 때는 `schema` 라벨 이슈로 알린다 (`CST-07`).
...
**서비스 경계를 넘는 FK ✅ (U10 확정)**: `trips`는 결제 DB에 있고 `trip_ids`는 정산 DB에 있으므로 **FK 참조 자체가 불가능하다.** 참조 없이 ID 값만 보관한다. 무결성은 DB가 아니라 `IF-01` 응답을 신뢰하는 것으로 대체되며, 존재하지 않는 `tripId`가 섞이면 대사(`FR-R-01`)에서 금액 차이로 드러난다.

## 다. Spring Batch 메타 테이블

`BATCH_JOB_INSTANCE` 등은 Spring Batch가 자동 생성하며, **정산 DB 인스턴스에 생성된다** (`ERD §2`). 소유권을 따질 필요 없이 물리적으로 정산 것이다. `FR-B-07`의 실패 재시작이 이 테이블에 의존한다.
